 ...
 * 
 * To use it, invoke startServentTest with a directory name as parameter. This
 * directory should include:
 * <ul>
 * <li>A <code>servent_list.properties</code> file (explained in
 * {@link AppConfig} class</li>
 ...
 * <code> servent0_in.txt </code>, <code>servent1_in.txt</code>, ... and so on
 * for each servent. These files should contain the commands for each servent,
 * as they would be entered in console.</li>
 * </ul>
 *
 * @author bmilojkovic
 */
public class MultipleServentStarter {
	...
	/**
	 * We will wait for user stop in a separate thread. The main thread is waiting
	 * for processes to end naturally.
	 */
	 ...
	 * contains a servent_list.properties file which will describe our distributed
	 * system.
	 */
	private static void startServentTest(String testName) {
		...
		for (int i = 0; i < serventCount; i++) {
			try {
				ProcessBuilder builder = new ProcessBuilder("java", "-cp", "out\\production\\KiDS-Domaci2-kostur",
